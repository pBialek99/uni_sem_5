...
2) odkomentować instrukcje: pthread_create i pthread_join,
   a wykomentować p(0) i q(0). Skompilować i uruchomić program kilka razy
...
Dlatego wątek główny nie powinien wywoływać return lub exit dopóki
...
/*****************-WNIOSKI/ODPOWIEDŹ-**************************
przypadek bez wątków - funkcje p(0) i q(0) nie wykonują się
współbieżnie tylko sekwencyjnie. Czyli najpierw wykonuje się
funkcja p(0) a następnie q(0).

przypadek z wątkami - funkcje p(0) i q(0) wykonują się
w odrębnych wątkach niezależnie od siebie ponieważ wątki
w1 oraz w2 wykonują swoje polecania oddzielnie.

pthread_join(...) - reguluje to, by wątek główny nie zakończył się
przed zakóńczeniem wątków w1 oraz w2.

ODPOWIEDŹ - program jest współbieżny podczas wykorzystania
pthread_create ponieważ wtedy funkcje wykonują się w osobnych
wątkach, czyli jeżeli jest to możliwe równocześnie.
***************************************************************/
